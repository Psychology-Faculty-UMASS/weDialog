...
/* Ivan.d - I think line below checks if User has answered questions in any topic of this dialog, and only shows Questions if NOT. This works good. But need to also add test if THIS Dialog has ANY questions, like isset(question1), and only show Questions, if YES.
If NO questions are set in THIS dialog, then do NOT show questionsanswers, just let user Reply, same as when user already answered questions. NOTE: there are multiple tests for question1 on this page - you can search to see them */
...
function reply_form_section2(reply_topc_id){
    ...
    if(user_id != '' && user_id > 0 && user_id != author){
    ...
    <div class="main_mid">
        ...
        <div class="topic1" id="show_tbl_detail" style="padding: 0 10px;">
                <div style="padding-bottom:13px;float:left;">
                    <?php if($TopicModel->user_id == Yii::app()->session['user_id'] || Yii::app()->session['group_id']==1 || Yii::app()->session['group_id']==3){ ?>
            ...
            <div>
                ...
                <p class="topic_desc_text" style="font-family: tahoma; width:100%; word-wrap: break-word;">
                    <?php if(!empty($TopicModel->topic_description)){
                        ...
                        echo "<span style='font-family:Arial;' class='top-desc-less-text'>".$topDesc."</span>";
                        //echo "<span style='font-size:14px; font-family:Arial;'>".HtmlTruncate::trim($TopicModel->topic_description, 450)."</span>";
                             if(strlen($TopicModel->topic_description) > 450){
                                ...
                                echo "<span style='display:none; font-family:Arial;' class='topic_desc_hidden_text'>".nl2br($TopicModel->topic_description)."</span>";
                ...
                <table style="width:98%;">
                    <tr style="font-family:Verdana;width:30px; margin:0px 0 0 0; ">
                        <td style="float:left;font-family:Verdana;width:99%">
                            <a style="font-family:Verdana;cursor: pointer;text-decoration: none;color:#000000;" href="javascript:void(0);" id="allmessage" onclick="threadcommentlisting(0);">THREAD &nbsp;|&nbsp;</a>
...
function threadcommentlisting(pagination){
    ...
    //var currect_section = "date_topics";
	if(need_ajax_call==1){
		data_str += "&record_cnt=30&last_comment_id="+last_comment_id+"&topic_id=<?php echo $TopicModel->id;?>";
...
<div id="green_open_form" style="display: none; width: 360px; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -249px; top: 15px;">
        <?php
        ...
        <table class="topic_detail" width="95%" border="0" class="tag" style="padding-left:1%;" cellpadding="0" cellspacing="0">
             <tr>
                <td style="padding-left: 1%;"><h4 class='h4-style-for-green-red-flags'>Reason for green flagging user:</h4></td>
            </tr>
...
<div id="red_open_form" style="display: none; width: 360px; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -249px; top: 15px;">
        <?php
            $form = $this->beginWidget('CActiveForm', array(
            		'id'=>'topic-form',
        ...
        <table class="topic_detail" width="95%" border="0" class="tag" cellpadding="0" cellspacing="0" style="padding-left:1%;">
            <tr>
                <td ><h4 class='h4-style-for-green-red-flags'>Reason for Red flagging user:</h4></td>
            </tr>
                <?php 
                    if(isset($flag_reason_model) && count($flag_reason_model) > 0){
                        $cnt = 1;
...
<div id="question_answer_open_form" style="display: none; width: 500px; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -249px; top: 15px;">
        <?php 
            $form = $this->beginWidget('CActiveForm', array(
        ...
        <table class="topic_detail" width="99%" border="0" class="tag" style="padding-left:5%;" cellpadding="0" cellspacing="0">
			...
			<tr><td><br/></td></tr>
			<tr>
			<td class="topic_detail" align="left">So everyone may better understand different perspectives, please indicate your:<a  style="right: 3px; margin-top: 10px;"></a></td
			</tr>
            <tr>
            <?php 
...
#rtl-support .tt-menu {
  text-align: right;
               ...
               <tr><td> <input class="typeahead" name="college" id="college" type="text" id="text" style="width: 80%;margin-top:0px; padding: 10px;" placeholder="Your University"><br></td><td></td></tr>
               <?php } ?>
               
               
               <tr><td>&nbsp;</td></tr>
                <tr>
                <td style="padding-left: 13%; text-align: center;">
                    <input type="button" name="save" value="OK" id="save" style="width: 30%; height: 40px; text-align: center;float:left;" onclick="questionvalidation();"/>
                    <input type="button" class="closemodel" name="Cancel" value="Cancel" id="closemodel" style="width: 30%; height: 40px;" onclick="javascript:closemodel_div('question_answer_open_form');" style="display: block;width: 30%;background:#d8d8d8!important;float: left;"/>
...
var states = [ <?php foreach($UnivModel as $model){ ?>'<?= str_replace("'","`",$model->college_name);?>',<?php } ?>];
...
function questionvalidation(){
    ...
    if(flag){ 
       ...
       var post_data = 'topic_id='+topic_id+'&post_id='+post_id+'&type='+type+'&question1='+question1+'&question2='+question2+'&college='+college+'&question3='+question3+'&answer1='+answer1+'&answer2='+answer2+'&answer3='+answer3;
