...
$(document).ready(function(){
    ...
    $("#tag-type-comment-form").submit(function(){
...
function likedislikecommentfun(comment_id, likedislike){
    ...
    if(user_id != '' && user_id > 0){
        if(comment_id==""){
            ...
            $.ajax ( {
                type: "POST",
                url: '<?php echo Yii::app()->createUrl('general/rulecount') ;?>',
                data: "comment_id="+comment_id+"&likedislike="+likedislike+"&post_type=2&main_id=<?php echo $tag_id;?>",
                success: function(response){
                    if(response == "fail"){
                        alert("Error");
    			    	...
    			    	$('#already_voted_message_'+comment_id).html("you are not authorized person to likedishlike.").style.display = "block";
    			    	return false;
                    }else{
                        if(likedislike == "like"){
...
<div class="main">
	<div style="text-align:center; float:left; width:100%"><?php $this->renderPartial('/partials/_flash_msgs'); ?></div>
    <div class="main_left" style="margin-bottom: 40px;">
        <?php $this->renderPartial('/topics/_topics_list_left_panel',$this->data);?>
		<?php
    	//$this->renderPartial('/topics/_topics_list_left_panel',$this->data);	// added by atul for left main menu 2015-12-23 05-32 PM
    	/*
    	?>
    ...
    <div class="main_mid">
        <!-- //============================START:  ABOUT TOPICS   =======================  -->
          <?php $this->renderPartial('/typeTags/_about_rule',$this->data);?>
        <!-- //============================END:  ABOUT TOPICS   =======================  -->
        <div class="topic1" id="show_tbl_detail" style="padding: 0 10px;">
            <div style="padding-bottom:13px;float:left;">
                <?php if($rule_model->user_id == Yii::app()->session['user_id'] || Yii::app()->session['group_id']==1 || Yii::app()->session['group_id']==3){ ?>
			
                        <a href="<?php echo Yii::app()->createUrl('TypeTags/Updatetypetags',array('tag_id'=>$rule_model->id))?>" style="text-transform:none;color: #FFFFFF;font-size: 13px; float: right;text-decoration: none;">
            ...
            <div style="clear: both;"></div>
            <div>
                <div class="rule_desc" style="color:#065A95;font-family: Tahoma;font-size: 17.5px;padding-bottom: 16px;cursor: pointer;" onclick="javascript: topicdetailmore2('<?php echo $rule_model->id; ?>')"><?php echo ucwords($rule_model->type_tag);?>
                <?php if(!empty($rule_model->type_tag_description)){ ?>
                    <img src="<?php echo Yii::app()->request->baseUrl;?>/images/info-icon.png" style="position: absolute; margin-left: 6px;"/>
                <?php } ?>
                </div>
                <p class="rule_desc_text" style="width:100%; word-wrap: break-word;">
                    <?php if(!empty($rule_model->type_tag_description)){
                         
                        $url = '@(http)?(s)?(://)?(([a-zA-Z])([-\w]+\.)+([^\s\.]+[^\s]*)+[^,.\s])@';
                        $ruleDesc = nl2br(myhelpers::get_cropped_text($rule_model->type_tag_description, 450));
                        if(strlen($rule_model->type_tag_description) > 450){
                            $ruleDesc = strip_tags($ruleDesc,'<br></br><br/>');
                        }
                        echo "<span class='rule-desc-less-text'>".$ruleDesc."</span>";
                        
                             if(strlen($rule_model->type_tag_description) > 450){
                                echo "<span class='rule_desc_more_dots'>...</span>";
                                $ruleDescMore = nl2br(myhelpers::get_cropped_more_text($rule_model->type_tag_description, 450));
                                echo "<span style='display:none; font-size:14px; font-family:Arial;' class='rule_desc_hidden_text'>".nl2br($rule_model->type_tag_description)."</span>";
                                echo "<a id='rule_desc_more'>More</a>";
                                echo "<a id='rule_desc_less' style='display:none'>Less</a>";
                                echo "<br/>";
                             }
                     } 
                     else{
                 ...
                 <?php if(!empty($rule_model->type_tag_description) && strlen($rule_model->type_tag_description) <= 450){ echo "<br/>"; } ?>
            </div>
                ...
                <table style="width:98%;">
                    <tr style="font-family:Verdana;width:30px; margin:0px 0 0 0; ">
                        <td style="float:left;font-family:Verdana" >
                            <a style="font-family:Verdana; cursor: pointer;text-decoration: none;color:#000000;" href="javascript:void(0);" id="allmessage" onclick="threadcomment('');">THREAD &nbsp;|&nbsp;</a>                            
                            ...
                            <a style="font-family:Verdana; cursor: pointer;text-decoration: none;color:#c3c3c3;" id="topmessage" onclick="javascript:change_current_section('popular_topics');">AGREE</a> 
                            <!--<a style="font-family:Verdana; cursor: pointer;text-decoration: none;color:#c3c3c3;" id="disagreemessage" onclick="javascript:change_current_section('disagree_topics');">DISAGREE &nbsp;</a>-->
                            <?php /*
                            <span id="mymessage">
                                <?php if(isset(Yii::app()->session['user_id']) && Yii::app()->session['user_id'] > 0){?>
                                <a style="font-family:Verdana; cursor: pointer;text-decoration: none;color:#c3c3c3;" onclick="javascript:change_current_section('my_topics');">|&nbsp;MY</a>
                                <?php }?>
                            </span>
                            */?>
                        </td>
    ...
    <div class="main_right">
        <!-- //============================START:  RIGHT PANEL   =======================  -->
          <?php $this->renderPartial('/typeTags/_right_panel',$this->data);?>
        <!-- //============================END:  RIGHT PANEL    =======================  -->
    </div>
</div>
<script>
...
<script>
var tag_id = <?php echo $this->data['tag_id'];?>;
...
function get_topic_comments(){
	...
	if(need_ajax_call==1){
		...
		if(ajax_call_complated==1 || is_fist_call == 1){
			is_fist_call = 0;
            $.ajax({
			url: "Getcomments?tag_id="+tag_id+"&selected_user_id="+selected_user_id,
			data: data_str,
			type: 'POST',
			dataType: "json",
			async:false,
...
<div id="green_open_form" style="display: none; width: 35%; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -249px; top: 200px;">
        <?php
            $form = $this->beginWidget('CActiveForm', array(
            		'id'=>'topic-form',
                    'action'=>Yii::app()->createUrl('TypeTags/creategreenflag'),
        ...
        <table class="topic_detail" width="95%" border="0" class="tag" style="padding-left:5%;" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                ?>
                    ...
                    <?php echo $form->hiddenField($user_comment_flag_model,'main_id',array("value"=>$tag_id));?>
...
<div id="red_open_form" style="display: none; width: 35%; position: fixed; opacity: 1; z-index: 11000; left: 50%; margin-left: -249px; top: 200px;">
        <?php
            $form = $this->beginWidget('CActiveForm', array(
            		'id'=>'topic-form',
                    'action'=>Yii::app()->createUrl('TypeTags/createredflag'),
            		'enableAjaxValidation'=>false,
            	    'enableClientValidation'=>true,
            	    'clientOptions'=>array(
            			'validateOnSubmit'=>true,
            ...
            <tr>
                ...
                ?>
                    ...
                    ?>       
                     ...
                     <tr><td>&nbsp;</td></tr>
                    <tr style="width: 100%;">
                        <td><?php echo $form->checkBox($user_comment_flag_model,'block_user',array('style'=>'width:5%'));?> Block User</td>
                    </tr>
                    <tr style="width: 100%;">
...
function saveReplay(comment_id,main_comment_id,main_id){
    ...
    if(comment!=''){
        $.ajax({
            type: 'POST', 
            url: "<?php echo Yii::app()->createUrl('TypeTags/submitreply');?>",
...
$("#rule_desc_more").click(function(){
    $(".rule_desc_more_dots").css("display","none");
    $(".rule_desc_hidden_text").css("display","inline");
    $("#rule_desc_more").css("display","none");
    $("#rule_desc_less").css("display","block");
    $(".rule-desc-less-text").css("display","none");
});

$("#rule_desc_less").click(function(){
    $(".rule_desc_more_dots").css("display","inline");
    $(".rule_desc_hidden_text").css("display","none");
    $("#rule_desc_more").css("display","block");
    $("#rule_desc_less").css("display","none");
    $(".rule-desc-less-text").css("display","inline");
});
