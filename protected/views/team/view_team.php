    ...
    <div class="main_mid">
        ...
        <div class="topic1" id="show_tbl_detail" style="padding: 0 10px;">
            <div style="padding-bottom:13px;float:left;">
                ...
                <?php if($team_model->user_id == Yii::app()->session['user_id'] || Yii::app()->session['group_id']==1 || Yii::app()->session['group_id']==3){ ?>
                        <a href="<?php echo Yii::app()->createUrl('Team/updateteam',array('id'=>$team_model->id)) ?>" 
						style="text-transform:none;color: #FFFFFF;font-size: 13px; float: right;text-decoration: none;">
            ...
            <div style="clear: both;"></div>
            <div>
                <div class="team_desc" style="color:#065A95;font-family: Tahoma;font-size: 17.5px;padding-bottom: 16px;cursor: pointer;" onclick="javascript: topicdetailmore2('<?php echo $team_model->id; ?>')"><?php echo ucwords($team_model->name);?>
                <?php if(!empty($team_model->description)){ ?>
                    <img src="<?php echo Yii::app()->request->baseUrl;?>/images/info-icon.png" style="position: absolute; margin-left: 6px;"/>
                <?php } ?>
                </div>
                <p class="team_desc_text" style="width:100%; word-wrap: break-word;">
                    <?php if(!empty($team_model->description)){
                         
                        $url = '@(http)?(s)?(://)?(([a-zA-Z])([-\w]+\.)+([^\s\.]+[^\s]*)+[^,.\s])@';
                        $teamDesc = nl2br(myhelpers::get_cropped_text($team_model->description, 450));
                        if(strlen($team_model->description) > 450){
                            $teamDesc = strip_tags($teamDesc,'<br></br><br/>');
                        }
                        echo "<span class='team-desc-less-text'>".$teamDesc."</span>";
                        
                             if(strlen($team_model->description) > 450){
                                echo "<span class='team_desc_more_dots'>...</span>";
                                $teamDescMore = nl2br(myhelpers::get_cropped_more_text($team_model->description, 450));
                                echo "<span style='display:none;  class='team_desc_hidden_text'>".nl2br($team_model->description)."</span>";
                                echo "<a id='team_desc_more'>More</a>";
                                echo "<a id='team_desc_less' style='display:none'>Less</a>";
                                echo "<br/>";
                             }
                     } 
                     else{
                 ...
                 <?php if(!empty($team_model->description) && strlen($team_model->description) <= 450){ echo "<br/>"; } ?>
            </div>
...
$("#team_desc_more").click(function(){
    $(".team_desc_more_dots").css("display","none");
    $(".team_desc_hidden_text").css("display","inline");
    $("#team_desc_more").css("display","none");
    $("#team_desc_less").css("display","block");
    $(".team-desc-less-text").css("display","none");
});

$("#team_desc_less").click(function(){
    $(".team_desc_more_dots").css("display","inline");
    $(".team_desc_hidden_text").css("display","none");
    $("#team_desc_more").css("display","block");
    $("#team_desc_less").css("display","none");
    $(".team-desc-less-text").css("display","inline");
});
