...
class TeamCommentFlagController extends Controller
{
    public $layout='//layouts/column2';
    
	public function actionAdmin()
	{
	   $model=new TeamCommentFlag('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['TeamCommentFlag']))
			$model->attributes=$_GET['TeamCommentFlag'];
    ...
    function actionManage_coment(){
        ...
        $modelname = 'TeamCommentFlag';

        if(!empty($_POST['action_type']) && $_POST['action_type']=="hide_post" && !empty($_POST['selected_ids'])){
		  $modeltemp=TeamCommentFlag::model()->findAll(array('condition'=>'id IN (' .$selected_ids. ')'));
			...
			$updated = TeamCommentFlag::model()->updateAll(array('flag_status'=>$new_status), 'id IN ('.$_POST['selected_ids'].') AND flag_status="'.$old_status.'"');
			if($updated){
            ...
            if(count($modeltemp)>0){
                ...
                {
                    $ids_userflagcoment[]=$modeltemp1->id;
                    $user_comment_ids[]=$modeltemp1->team_comment_id;
                }
                $ids_userflagcoment=implode(',',$ids_userflagcoment);
		...
		$this->redirect(Yii::app()->createUrl('TeamCommentFlag/admin'));
	}
