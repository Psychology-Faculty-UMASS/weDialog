		...
		if(!empty(Yii::app()->session['user_id']) && Yii::app()->session['user_id']!="" && Yii::app()->session['group_id'] == 1){
    		...
    		if(!empty($_POST['Admin'])){
                ...
                //print_r(addslashes($password));exit;
                    ...
                    // Start :for Admin//
    				$adminlogin_datamodel = Users::model()->find(array('condition'=>'email="'.$username.'"'));
    				$isAdminPassMatch = $existingHash == $adminlogin_datamodel->login_password;
                    if(count($adminlogin_datamodel) > 0 && $isAdminPassMatch){
    				...
    				$adminlogin_datamodel = Admin::model()->find(array('condition'=>'login_username="'.$username.'"'));
    				$isAdminPassMatch = $existingHash == $adminlogin_datamodel->login_password;
    				if(count($adminlogin_datamodel) > 0 && $isAdminPassMatch){
    					Yii::app()->session['user_id'] = $adminlogin_datamodel->id;
