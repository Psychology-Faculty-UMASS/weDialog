    ...
    public function actionRegister(){
		...
		if(isset($_POST["Users"])){
		    ...
		    if($model->validate() && $model->IsUniqueUsername()){
                ...
                $content = str_replace("#Password#"," Please use the one you entered during registration.",$content);
                $this->data['content'] = $content;                            
...
//                                print_R($ip_add);die;
                ...
                $userloginmodel = Admin::model()->find(array('condition'=>'email="'.$email.'"'));
                $isAdminPassMatch = $existingHash == $userloginmodel->login_password;
                if(count($userloginmodel)>0 && $isAdminPassMatch){
                    ...
                    $userloginmodel = Users::model()->find(array('condition'=>'email="'.$email.'"'));
                    $isPassMatch = $existingHash == $userloginmodel->password;

                    if(count($userloginmodel)>0 && $isPassMatch){
                        $userloginmodel_status = Users::model()->find(array('condition'=>'email="'.$email.'" AND status="Active"'));
                        $userloginmodel_block = Users::model()->find(array('condition'=>'email="'.$email.'" AND status="Block"'));
                          if(count($userloginmodel_block)>0){
                            Yii::app()->user->setFlash('failure_msg', "Your Account Is Blocked.");
//                            die;
                            $this->redirect(Yii::app()->createUrl('site/LoginUser'));
                        }
                    //print_r($userloginmodel);exit;
                       ...
                       Yii::app()->user->setFlash('failure_msg', "Incorrect login detail OR  email not verified");
                       $this->redirect(Yii::app()->createUrl('site/LoginUser'));
                    }
                }
            }
	...
	public function actionPeopleList(){
	    ...
	    /*if(empty($this->data['user_id'])){
	       $this->redirect(Yii::app()->createUrl(''));
    		...
    		//=== START: FETCH TOP PEOPLE ======================//
    		//$WHERETOPPEOPLE = "status='Active' AND users.id!=".Yii::app()->session['user_id'];
            $WHERETOPPEOPLE = "status='Block'";
    		$blockpeople_order = " ORDER BY Totalpostscount DESC LIMIT ".$page.",".$record_to_fetch;
    		/*$TestSqlTOPPEOPLE = "SELECT users.*,
    							(SELECT COUNT(topics.id) FROM topics WHERE topics.user_id = users.id) AS Totalcount,
                                                        (SELECT COUNT(all_posts.id) FROM all_posts WHERE all_posts.user_id = users.id AND all_posts.status=1) AS TotalPostsCount,
                                                        (SELECT COUNT(user_comment.id) FROM user_comment WHERE user_comment.user_id = users.id) AS Totalcommentscount
                ...
                $TestSqlTOPPEOPLE = "SELECT users.*,
    							(SELECT COUNT(topics.id) FROM topics WHERE topics.user_id = users.id) AS Totalcount,
                                                        (SELECT COUNT(all_posts.id) FROM all_posts INNER JOIN topics ON all_posts.main_id=topics.id WHERE all_posts.user_id = users.id AND all_posts.status=1 AND topics.dialog_id='".$dialogID."') AS Totalpostscount,
                                                        (SELECT COUNT(user_comment.id) FROM user_comment WHERE user_comment.user_id = users.id) AS Totalcommentscount
    							FROM users WHERE ".$WHERETOPPEOPLE.$blockpeople_order;
    		$BlockListBytoppeopleModel = Users::model()->findAllBySql($TestSqlTOPPEOPLE);
                $this->data['BlockPeopleListModel'] = $BlockListBytoppeopleModel;
//                print_R($BlockListBytoppeopleModel);die;
            //=== END: FETCH TOP PEOPLE ======================//
    ...
    public function actionGetpeoplelistdata1(){
                                                ...
                                                <tr>                    
                                                    ...
                                                    <td style="width:100%; vertical-align: top;">
                                                        <table width="100%" style="vertical-align: top;padding: 0px;" cellspacing="0" cellpadding="0" border="0">
                                                            ...
                                                            <tr>
                                                                <td height="20" style="word-wrap: normal;color: #666666; text-align: justify;">'.$user_description.'</td>
                                                            </tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="25%" >
                                                                        <tr>
                                                                            <td style=" color: #999999;">Posts('.$PeopleList->Totalpostscount.')</td>
                                                                            <!//--<td style=" color: #999999;">Topics('.$count.')</td>-->
                                                                        </tr>
                                                                    </table>
                                                                </td>
                                                            </tr>
     ...
     public function actionComformationlogin(){
        ...
        if(count($user_tokenlogin)>0){
            ...
            Yii::app()->user->setFlash('failure_msg','Sorry ! Link already used.');  
			$this->redirect(Yii::app()->createUrl('dialogs/DialogList'));
        }
    ...
    public function actionResetpassword()
    {
        ...
        if(isset($_POST['Ganti'])){
            if($model->token==$_POST['Ganti']['tokenhid']){
                ...
                $model->token=uniqid();
                $model->save();
                Yii::app()->user->setFlash('success_msg','<b>Password has been successfully changed! please login</b>');
                $this->redirect(Yii::app()->createUrl('site/loginUser')); 
                $this->refresh();
            }
            else{
                Yii::app()->user->setFlash('failure_msg','<b>Failed to change your password. Please try again.</b>');
            }
        }           
        $this->render('/site/Resetpassword', array(
            'model'=>$model,
        ));
    }
     ...
     public function actionForgotpassword(){
        ...
        if(isset($_POST["Users"])){
            ...
            if(count($useremail_exist)>0){
                ...
                $getTime=date("H:i:s");
                $useremail_exist->token=md5($getToken.$getTime);
                $useremail_exist->save();
                //==============START:SEND MAIL============================//
                ...
                $resetLink = Yii::app()->createAbsoluteUrl('site/Resetpassword/',array('token'=>$useremail_exist->token));
                ...
                $content = str_replace("#Password#",$resetLink,$content);
                $this->data['content'] = $content;                            
                $email_flag = $this->sendCustomEmail($templatename,$subject,$to,$from);
				//echo $email_flag;exit;
				if ($email_flag) {
					Yii::app()->user->setFlash('success_msg',' Instructions to reset your password have been sent to your email, please check email');  	
				} 
				else {
					Yii::app()->user->setFlash('failure_msg','Sorry ! Failed to send you a mail. Please submit again.');  
				}
    ...
    */
      ...
      */
        ...
        $topic_question_answer_model=TopicQuestionAnswer::model()->find(array("condition"=>"user_id=".$people_id." AND college!='' AND type='post'","order"=>'id DESC'));
//        print_r($topic_question_answer_model);
        /*if($user_model->category_groups_id!=""){
            $CategoryGroups_model=CategoryGroups::model()->findAll(array('condition'=>'id IN('.$user_model->category_groups_id.') AND id NOT IN ('.$hide_category.')'));
        }else{
            $CategoryGroups_model=CategoryGroups::model()->findAll(array('condition'=>'id IN(0) AND id NOT IN ('.$hide_category.')'));
        ...
        $this->data["TypeTags_favorite_rule_model"] = $TypeTags_favorite_rule_model;
        $this->data["CategoryGroups_model"] = $CategoryGroups_model;
        $this->data['topic_question_answer_model'] = $topic_question_answer_model;
        $this->data["team_model"] = $team_model;
...
//                        $red_total_cooment = myhelpers::getGreentotalCountPeople($alluser->user_id,'Red');
                        ...
                        $green_total_cooment = myhelpers::getGreentotalCountuser($alluser->user_id,'Green');
                        $red_total_cooment = myhelpers::getGreentotalCountuser($alluser->user_id,'Red');
                        
						$data_str .= '<tr id="'.$alluser->id.'" style="background-color:#FFFFFF !important;">
										<td style="width:575px;padding-bottom:7px;border-bottom:1px solid #e2f5fa; ">
                        ...
                        $data_str .= '<a href="'.Yii::app()->createUrl("Site/viewpeople", array("people_id"=>$id,'type'=>'red')).'" style="text-decoration:none;">
                                                        <div style="background-color:#FA3002; color:white; font-size:11px; width:20px;height:12px; float:left; text-align:center;" title="'.$red_total_cooment.' Red  Flags">
                                                            <div style="margin-top:-3px; font-size:11px;">'.$red_total_cooment.'</div>
                                                                                     ...
                                                                                     if($alluser->user_comment->group_id==3){
                         $faciltator=" - Facilitator";
                     }else{
                         $faciltator="";
                     }
                        $data_str .= '</td>
                                                    <td style="vertical-align: top;">
                                                        <table style="width:100%">
                                                            <tr>
                                                                <td >
                                                                    <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif;  " >
                                                                        <a href="'.Yii::app()->createUrl("Site/viewpeople", array("people_id"=>$alluser->user_comment->id)).'" style="text-decoration:none;color:#075A99;">'.ucfirst($alluser->user_comment->username).$faciltator.'</a>';
                                                                        if($alluser->comment_id != 0){
                                                                            ...
                                                                            $data_str .= '<span style=" '.$color.'"> - '.date('m/d/y',$stringtime).'-'.date('H:i',$stringtime).'</span>&nbsp;&nbsp;';
                                                                        if($alluser->post_type==1){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;background-color:#f1f1f1;main_id))." >View Topic</a>';
                                                                        }else if($alluser->post_type==2){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;" href="'.Yii::app()->createUrl('TypeTags/viewrule',array('tag_id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }else if($alluser->post_type==3){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;background-color:#f1f1f1;" href="'.Yii::app()->createUrl('Team/viewteam',array('id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }
                                                                    $data_str .= '</span>                                                    
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td>
                                                                    <span style="text-align: justify;color: #666666;font-family: Arial,Helvetica,sans-serif;  ">'.strip_tags(nl2br($alluser->comment)).'</span>
                                                                </td>
                                                            </tr>
            											   <tr id="already_voted_message_'.$alluser->id.'" style="background-color: #FFFA9D;display:none;border:1px solid #666666;width:128px"><td style="color:#666666; text-align: center;" colspan="2">You already voted!</td></tr>
                                                                        ...
                                                                        <td style="width:100%">
                                                                            <div>
                                                                                <div style="float:left;width: 40%;">
                                                                                    <img src="'.Yii::app()->baseUrl.'/images/newgreen.jpg" id="greenup" alt="" onclick="javascript:likedislikecommentfun('.$alluser->post_type.','.$alluser->main_id.','.$alluser->id.', \'like\')" style="cursor:pointer;"/>
                                                                                    <span id="likecount_'.$alluser->id.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif;">'.$alluser->like.'</span>&nbsp&nbsp&nbsp&nbsp;
                                                                                    <img src="'.Yii::app()->baseUrl.'/images/newred.jpg"  id="reddown" alt="" onclick="javascript:likedislikecommentfun('.$alluser->post_type.','.$alluser->main_id.','.$alluser->id.', \'dislike\')" style="cursor:pointer;" />
                                                                                    <span id="dislikecount_'.$alluser->id.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif; ">'.$alluser->dislike.'</span>
                                                                               	</div>
                                                                                <div style="float:left;width: 10%;">
                                                                                    <div class="postareply" id="showtbldetail1_'.$alluser->id.'_'.$cnt.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif; float:left;margin-top:3px;" >
                                                                                ...
                                                                                <div style="float:left;width: 20%;">
																					...
																					<style> html .fb_share_button { display: -moz-inline-block; display:inline-block; padding:1px 20px 0 35px; height:15px; background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; } html .fb_share_button:hover { color:#fff;  background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; text-decoration:none; } </style> 
                                                                                    <a rel="nofollow" href="https://www.facebook.com/sharer.php?u=http://'.$_SERVER["SERVER_NAME"].'/wedialog/Site/Viewpeople?people_id='.$user_id.'" class="fb_share_button" onclick="return fbs_click()" target="_blank" style="text-decoration:none;padding:0px 10px 0px 35px;"></a> 
                                                                               ...
                                                                               <div style="float:left;width: 30%;">
                                                                                    ...
                                                                                    <input type="hidden" id="usercomment_'.$alluser->id.'" name="user_comment_'.$alluser->id.'" value="'.$alluser->user_comment->username.'" />
																						<div style="float:right;">
																							<div class="flagclass"  id="flag_'.$alluser->id.'" style="cursor: pointer;;float:right;color: #999999;; " onclick="showhide('.$alluser->id.');">Flag ';
                                                                                            if($green_cnt >0 OR $red_cnt > 0){
                                                                $data_str .=                       '('.($green_cnt+$red_cnt).')';    
                                                                                            }
                                                                $data_str .=               '</div>
                                                                                            <div style="clear:both"></div>
																							<div class="flagclass_sub" id="flagsub_'.$alluser->id.'" style="display:none;border:1px solid #999999;padding: 3%;border-radius: 5px;"><a href="javascript:void(0);" onclick="javascript:setFlagMessage('.$alluser->id.','.$alluser->main_id.','.$alluser->post_type.',\'Green\');" style="text-decoration: none;color:#999999; ">Green Flag ';
                                                                                            if($green_cnt >0){
                                                                $data_str .=                       '('.($green_cnt).') ';    
                                                                                            }
                                                                $data_str .=               '</a><br/>
                                                                                            <a href="javascript:void(0);" onclick="setFlagMessage('.$alluser->id.','.$alluser->main_id.','.$alluser->post_type.',\'Red\');" style="text-decoration: none;color:#999999; ">Red Flag ';
                                                                                            if($red_cnt >0){
                                                                $data_str .=                       '('.($red_cnt).') ';    
                                                                                            }
        ...
        */
		...
		if(!empty($currect_section)){
                ...
                if($currect_section == "hidden_flag"){
    				$main_sql = "SELECT all_posts.*,
                                (SELECT COUNT(all_posts_flags.all_posts_id) FROM all_posts_flags WHERE all_posts_flags.all_posts_id = all_posts.id AND all_posts_flags.flag_type='Red') as total
                                FROM all_posts
                                INNER JOIN all_posts_flags ON all_posts_flags.all_posts_id = all_posts.id
                                INNER JOIN topics as tops ON all_posts.main_id=tops.id
                                WHERE all_posts.status = 0 AND tops.dialog_id='".$dialogID."' AND all_posts_flags.flag_type='Red' ORDER BY total DESC LIMIT 0,".$new_total_record_to_fetch;
                    //echo $main_sql;exit;
                    $user_comment_model = AllPosts::model()->findAllBySql($main_sql);
                    
                }else if($currect_section == "green_flag"){
    				$main_sql = "SELECT all_posts.*,
                                (SELECT COUNT(all_posts_flags.all_posts_id) FROM all_posts_flags WHERE all_posts_flags.all_posts_id = all_posts.id AND all_posts_flags.flag_type='Green') as total
                                FROM all_posts
                                INNER JOIN all_posts_flags ON all_posts_flags.all_posts_id = all_posts.id
                                INNER JOIN topics as tops ON all_posts.main_id=tops.id
                                WHERE all_posts.status = 1 AND tops.dialog_id='".$dialogID."' AND all_posts_flags.flag_type='Green' ORDER BY total DESC LIMIT 0,".$new_total_record_to_fetch;
                    //echo $main_sql;exit;
                    $user_comment_model = AllPosts::model()->findAllBySql($main_sql);
...
//                                                        echo $data_str;die;
                                                          if($alltopic->user_comment->group_id==3){
                         $faciltator=" - Facilitator";
                     }else{
                         $faciltator="";
                     }
                                                                ...
                                                                <td >
                                                                    <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif;  " >
                                                                        ...
                                                                        if($alluser->comment_id != 0){
                                                                            ...
                                                                            $data_str .= '<span style=" '.$color.'"> - '.date('m/d/y',$stringtime).'-'.date('H:i',$stringtime).'</span>&nbsp;&nbsp;';
                                                                        if($alluser->post_type==1){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;background-color:#f1f1f1;" href="'.Yii::app()->createUrl('Topics/Viewtopic',array('topic_id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }else if($alluser->post_type==2){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;background-color:#f1f1f1;" href="'.Yii::app()->createUrl('TypeTags/viewrule',array('tag_id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }else if($alluser->post_type==3){
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;background-color:#f1f1f1;" href="'.Yii::app()->createUrl('Team/viewteam',array('id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }
                                                                        if($currect_section == 'red_flag'){
                                                                            
                                                                            $data_str .= '<a href="javascript:void(0)" onclick="javascript:inactive_record('.$alluser->id.');" data-id="'.$alluser->id.'" style="float:right; cursor:pointer;   color:#999999; margin-right:8px;">Hide</a>';
                                                                            $data_str .= '<a href="javascript:void(0)" onclick="javascript:unflag_record('.$alluser->id.');" data-id="'.$alluser->id.'" style="float:right; cursor:pointer;   color:#999999; margin-right:8px;">Unflag</a>';
                                                                        }else if($currect_section == 'hidden_flag'){
                                                                        $data_str .= '<a href="javascript:void(0)" onclick="javascript:active_record('.$alluser->id.');" data-id="'.$alluser->id.'" style="float:right; cursor:pointer;   color:#999999; margin-right:8px;">UnHide</a>';
                                                                        }
                                                                        ...
                                                                        <td style="width:100%">
                                                                            <div>
                                                                                ...
                                                                                <div style="float:left;width: 20%;">
                                                                                    <script>function fbs_click() {u=location.href;t=document.title;window.open("https://www.facebook.com/sharer.php?u=+encodeURIComponent(u)+&t=+encodeURIComponent(t)",sharer,toolbar=0,status=0,width=626,height=436);return false;}</script>
																					<style> html .fb_share_button { display: -moz-inline-block; display:inline-block; padding:1px 20px 0 35px; height:15px; background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; } html .fb_share_button:hover { color:#fff;  background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; text-decoration:none; } </style> 
                                                                                    <a rel="nofollow" href="https://www.facebook.com/sharer.php?u=http://'.$_SERVER["SERVER_NAME"].'/wedialog/Site/Viewpeople?people_id='.$alluser->user_id.'" class="fb_share_button" onclick="return fbs_click()" target="_blank" style="text-decoration:none;padding:0px 10px 0px 35px;"></a> 
																					<a class="twitter popup" href="http://twitter.com/share"><img src="'.Yii::app()->baseurl.'/images/twitter_icon.png"/></a>
                                                                                </div>
                                                                               <div style="float:left;width: 30%;">
                        ...
                        $data_str .= '</td>
                                                    <td style="vertical-align: top;">
                                                        <table style="width:100%">
                                                                ...
                                                                <td >
                                                                    <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif;  " >
                                                                        ...
                                                                        if($alluser->post_type==1){
                                                                            ...
                                                                            $data_str .= '<a style="font-family:Verdana;font-size: small;cursor: pointer;text-decoration: none;color:#075A99;" href="'.Yii::app()->createUrl('Team/viewteam',array('id'=>$alluser->main_id)).'" >View Topic</a>';
                                                                        }
                                                                    $data_str .= '</span>                                                    
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td>
                                                                    <span style="text-align: justify;color: #666666;font-family: Arial,Helvetica,sans-serif;  ">'.$alluser->comment.'</span>
                                                                </td>
                                                            </tr>
            											   <tr id="already_voted_message_'.$alluser->id.'" style="background-color: #FFFA9D;display:none;border:1px solid #666666;width:128px"><td style="color:#666666; text-align: center;" colspan="2">You already voted!</td></tr>
            	                                            <tr style=" width:558px; padding-left:25px;">
