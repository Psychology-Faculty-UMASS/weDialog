...
class AllPostsController extends Controller
{
    ...
    public function actionInactiveRecord()
    {
        if(isset($_POST['id']))
        {
            $id=$_POST['id'];
            $allposts = AllPosts::model()->findByPk($id);
            $allposts->status=0;
            $allposts->update(array('status'));
            echo 1;
        }
        if(isset($_POST['unhide_id']))
        {
            $id=$_POST['unhide_id'];
            $allposts = AllPosts::model()->findByPk($id);
            $allposts->status=1;
          ...
          if(isset($_POST['remove_red_flag']))
        {
              $id=$_POST['all_posts_id'];
            echo $deleted = AllPostsFlags::model()->deleteAll("all_posts_id = $id and flag_type ='red' ");
        }
          if(isset($_POST['block_user']))
        {
              $id=$_POST['block_user'];
//            echo $deleted = AllPostsFlags::model()->deleteAll("all_posts_id = $id");
            $updated = Users::model()->updateAll(array('status'=>"Block"), 'id = '.$id);
        }
          if(isset($_POST['unblock_user']))
        {
              $id=$_POST['unblock_user'];
//            echo $deleted = AllPostsFlags::model()->deleteAll("all_posts_id = $id");
            $updated = Users::model()->updateAll(array('status'=>"Active"), 'id = '.$id);
        }
    } 
