...
/*Ivan.c - I think below sql needs to be changed to Select only people with NumberRedFlag>0, who have also Posted in THIS dialog.
I beleve NumberRedFlage per user is given by Totalcount. Correct value for NumberRedFlag is also given in red_flag column 
in Admin/Users. Also TOPPEPEPLE Sort, above gives correct People who have posted in THIS dialag 
I changed in $TestSqlREDFLAGPEOPLE first select to commented_by=user_id, from user_id=user.id */

			$TestSqlREDFLAGPEOPLE="select *,(select count(flag_type) from all_posts_flags where flag_type='Red' AND commented_by=users.id) as Totalcount, (SELECT COUNT(all_posts.id) FROM all_posts INNER JOIN topics ON all_posts.main_id=topics.id WHERE all_posts.user_id = users.id AND all_posts.status=1 AND topics.dialog_id='".$dialogID."') AS Totalpostscount from users where ".$WHEREREDFLAGPEOPLE.$redflagpeople_order;
    ...
    */
    ...
    public function actionTopposts($dialog_id="",$type=''){
			...
			if(count($user_model)>0){
                ...
                if($currect_section == "green_flag"){
                    ...
                    $main_sql = "SELECT all_posts.*,
                                ...
                                FROM all_posts
                                INNER JOIN all_posts_flags ON all_posts_flags.all_posts_id = all_posts.id
                                WHERE all_posts.user_id=".$user_id." AND all_posts.status = 1 AND all_posts_flags.flag_type='Red' ORDER BY total DESC LIMIT 0,".$new_total_record_to_fetch;
                    $user_comment_model = AllPosts::model()->findAllBySql($main_sql);
        ...
        */
                                                                ...
                                                                <td>
                                                                    <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif; font-size: 14px;" >
                                                                        ...
                                                                        if($currect_section == 'red_flag'){
                                                                            $data_str .= '<a href="javascript:void(0)" onclick="javascript:inactive_record('.$alluser->id.');" data-id="'.$alluser->id.'" style="float:right; cursor:pointer; font-size: 14px; color:#999999; margin-right:8px;">Hide</a>';
                                                                        }
                                                                    $data_str .= '</span>                                                    
                                                                </td>
                                                            </tr>
		...
		$response_aray['response_data_str'] = $data_str;
		$response_aray['no_more_data'] = $no_more_data;
                $response_aray['topSecCount'] = (int)$topSecCount;
		print_r(json_encode($response_aray));exit;
	}
    ...
    public function actionCreateredflag(){
        ...
        if(isset($_POST['AllPostsFlags'])){
            ...
            if($model_check > 0)
            {
                 Yii::app()->user->setFlash('failure_msg',"You have already flagged");
                 $this->redirect(CHttpRequest::getUrlReferrer());
            }else{
                $usermodel_check=AllPosts::model()->count("id=".$_POST['AllPostsFlags']['all_posts_id']." AND user_id=".$_POST['AllPostsFlags']['user_id']." AND post_type=".$_POST['AllPostsFlags']['post_type']);
                if($usermodel_check > 0){
