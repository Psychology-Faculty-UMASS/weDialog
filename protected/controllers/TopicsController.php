	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Topics']))
		{
            ...
            if(empty($model->created_date)){
				$model->created_date = date("m-d-y H:i:s");
			}
...
/*Ivan.b - this may be where changing Post status is done, since I see "chiled", or "child", belwo, which may be Reply post.
So may need code that says "Do not show Reply(Child) if Post Status=Inactive in Admin-Post". If this doesn't go here, must go somewhere
else on this page, since code already correctly hides Posts(Parent) posts in THREAD, and also hide Posts in NEW and AGREE sort */
        ...
        if(isset(Yii::app()->session['user_id']) && Yii::app()->session['user_id'] > 0){
            ...
            */
            ...
            if(isset($_POST["Topics"])){
                ...
                //$TopicModel->created_date = date("m-d-y H:i:s");
                if(!empty(Yii::app()->session['dialog_id'])) {
...
//                $TopicModel->topic_description = $purifier->purify($TopicModel->topic_description);
    ...
    public function actionViewtopic(){
        ...
        $UnivSql = "SELECT college_name from colleges";
        $UnivModel = Colleges::model()->findAllBySql($UnivSql);
        $this->data["UnivModel"] = $UnivModel;
//print_R($UnivModel);die;
                        ...
                        $data_str .= '</td>
                                                    <td style="vertical-align: top;">
                                                        <table style="width:100%">
            											   ...
            											   <tr id="already_voted_message_'.$alltopic->id.'" style="background-color: #FFFA9D;display:none;border:1px solid #666666;width:128px"><td style="color:#666666;font-size: 15px;text-align: center;" colspan="2">You already voted!</td></tr>
                                                                    ...
                                                                    <tr style="float:right;width:100%;">
                                                                       ...
                                                                       $data_str .='
                                                                                <div style="float:left; width:40%;">
                                                                                    ...
                                                                                    <span id="likecount_'.$alltopic->id.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif;">'.$green_vote_count.'</span>&nbsp&nbsp&nbsp&nbsp;
                                                                                    ...
                                                                                    <span id="dislikecount_'.$alltopic->id.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif; ">'.$red_vote_count.'</span>
                                                                               	</div>
                                                                                <div style="float:left;width: 10%;">
                                                                                    <div class="postareply" id="showtbldetail1_'.$alltopic->id.'_'.$cnt.'" style="color: #999999;font-family: Arial,Helvetica,sans-serif; float:left;" >
                                                                                    	<span style="cursor:pointer; " onclick="javascript:reply_form_section(\''.$alltopic->id.'\');" id="reply_'.$alltopic->id.'">Reply</span>
                                                                                    </div>
                                                                                </div>
                                                                                <div class="post-share" style="float:left;width: 20%;">
                                                                                    <script>function fbs_click() {u=location.href;t=document.title;window.open("https://www.facebook.com/sharer.php?u=+encodeURIComponent(u)+&t=+encodeURIComponent(t)",sharer,toolbar=0,status=0,width=626,height=436);return false;}</script>
																					<style> html .fb_share_button { display: -moz-inline-block; display:inline-block; padding:1px 20px 0 5px; height:15px;   background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; } html .fb_share_button:hover { color:#fff; border-color:#295582; background:url('.Yii::app()->baseUrl.'/images/facebook_share_icon.gif) no-repeat top right; text-decoration:none; } </style> 
                                                                                    <a rel="nofollow" href="https://www.facebook.com/sharer.php?u=http://'.$_SERVER["SERVER_NAME"].'/wedialog/topics/Viewtopic?topic_id='.$topic_id.'" class="fb_share_button" onclick="return fbs_click()" target="_blank" style="text-decoration:none; padding:0px 10px 0px 35px;"></a> 
                                                                               ...
                                                                               <div style="float:left;width: 30%;">
                                                                                    ...
                                                                                    <input type="hidden" id="usercomment_'.$alltopic->id.'" name="user_comment_'.$alltopic->id.'" value="'.$alltopic->user_comment->username.'" />
																						<div style="float:right;">
																							<div class="flagclass"  id="flag_'.$alltopic->id.'" style="cursor: pointer;;float:right;color: #999999;" onclick="showhide('.$alltopic->id.');">Flag ';
                                                                                            if($green_cnt >0 OR $red_cnt > 0){
                                                                $data_str .=                       '('.($green_cnt+$red_cnt).')';    
                                                                                            }
                                                                $data_str .=               '</div>
                                                                                            <div style="clear:both"></div>
																							<div class="flagclass_sub" id="flagsub_'.$alltopic->id.'" style="display:none;border:1px solid #999999;padding: 3%;border-radius: 5px;"><a href="javascript:void(0);" onclick="javascript:setFlagMessage('.$alltopic->id.',\'Green\');" style="text-decoration: none;color:green; font-color:"green";">&#9607 Green ';
                                                                                             if($green_cnt >0){
                                                                $data_str .=                       '('.($green_cnt).') ';    
                                                                                            }
                                                                $data_str .=               '</a><br/>
                                                                                            <a href="javascript:void(0);" onclick="setFlagMessage('.$alltopic->id.',\'Red\');" style="text-decoration: none;color:#999999; ;font-color:"red";">&#9607 Red ';
                                                                                            if($red_cnt >0){
                                                                $data_str .=                       '('.($red_cnt).') ';    
                                                                                            }
            															...
            															<table style="width:100%; vertical-align: top;">
            																<tr>
            																	<td id="reply_comment_id_'.$alltopic->id.'">
            																		<textarea id="replycomment_'.$alltopic->id.'" name="replycomment_'.$alltopic->id.'" style="width:100%; height:250px;font-family:Arial,Helvetica,Tahoma,sans-serif; font-size: 15px; padding: 1%;"></textarea>
            																	</td>
            																</tr>
            																<tr>
            																	<td>
...
//                                    $topic_question_answer_sql = "SELECT college FROM topic_question_answer WHERE post_id =".$alltopic->id;
//                                            echo $alltopic->id."-----";
                                            $topic_question_answer_model=TopicQuestionAnswer::model()->find(array("condition"=>"post_id=".$alltopic->id,"order"=>'id DESC'));
//                                            echo $topic_question_answer_model->post_id;
					    /*Start For filter to display only liked and disliked users post for Vote filter only */
                        if(isset(Yii::app()->session['filter']['vote'])){
                                    $temp_status=1;                                    
                                    $filter_user_ids_array=explode(",",$filter_user_ids);
...
//                        $red_total_cooment = myhelpers::getGreentotalCount($alltopic->main_id,$alltopic->user_id,'Red','1');
                        ...
                        $green_total_cooment = myhelpers::getGreentotalCountuser($alltopic->user_id,'Green');
                        $red_total_cooment = myhelpers::getGreentotalCountuser($alltopic->user_id,'Red');
                        ...
                        if(!empty($alltopic->comment)){
                            ...
                            $commentMore.= "<span style='text-align: justify;color: #666666;font-family: Arial,Helvetica,sans-serif; word-break: break-word;' id='comment-less-txt-".$alltopic->id."'>".$comText."</span>";
                                if(strlen($alltopic->comment) > 700){
                                   ...
                                   $commentMore.= "<span style='display:none; text-align: justify;color: #666666;font-family: Arial,Helvetica,sans-serif;' class='comment_desc_hidden_text_".$alltopic->id."'>".nl2br($alltopic->comment)."</span>";
                                                    ...
                                                    <td style="vertical-align: top;">
                                                        <table style="width:100%">
                                                            <tr>
                                                                <td>
                                                                    <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif;" >
                                                                        <a target="_blank" href="'.Yii::app()->createUrl("Site/viewpeople", array("people_id"=>$alltopic->user_comment->id)).'" style="text-decoration:none;color:#075A99;">'.ucfirst($alltopic->user_comment->username).$faciltator.'</a>';
                                                                        if($alltopic->comment_id != 0){
                                                                            $data_str .= '<span style="color:#065A95;"> > @ </span><a target="_blank" href="'.Yii::app()->createUrl("Site/viewpeople", array("people_id"=>$alltopic->topic_other_comment->user_comment->id)).'" style="text-decoration:none;color:#075A99;">'.ucfirst($alltopic->topic_other_comment->user_comment->username).'</a>';
                                                                        }
                                                                        $data_str .= '<span style="'.$color.'"> - '.date('m/d/y',$stringtime).'-'.date('H:i',$stringtime).'</span>';
                                                                        //$data_str .= '<span style="font-size: 14px;float:right">'.$topic_question_answer_model->college.'</span>';
                                                                        
                                                                        if(Yii::app()->session['user_id']==$alltopic->user_id){
                                                                            $data_str .= '<span class="c-edit-post-button" data-id="'.$alltopic->id.'" style="float:right; cursor:pointer; color:#999999; margin-right:8px;">Edit</span>';
...
Same needs to also work for Hide Reply, below*/
                                                            ...
                                                            <tr style="display:none;" data-show="none" class="c-post-text-edit-form" id="c-post-text-edit-form-'.$alltopic->id.'" data-id="'.$alltopic->id.'">
                                                                <td>
                                                                    <textarea id="c-post-text-edit-comment-area-'.$alltopic->id.'" name="c-post-text-edit-comment-area" style="width:100%; height:250px; resize: none; font-family: Arial,Helvetica,Tahoma,sans-serif;">'.strip_tags($alltopic->comment,'<br></br><br/>').'</textarea>
                                                                    <br/>
                                                                    <input value="Post" class="type c-post-text-submit-post" data-id="'.$alltopic->id.'" style="float: right;" type="submit">
                                                                </td>
                                                            </tr>
            											   <tr id="already_voted_message_'.$alltopic->id.'" style="background-color: #FFFA9D;display:none;border:1px solid #666666;width:128px"><td style="color:#666666;text-align: center;" colspan="2">You already voted!</td></tr>
            	                                            <tr style=" width:558px; padding-left:25px;">
                                                                ...
                                                                $data_str .=               '</a><br/>
                                                                                            <a href="javascript:void(0);" onclick="setFlagMessage('.$alltopic->id.',\'Red\');" style="text-decoration: none;color:red; ; font-color:"red";">&#9607 Red ';
                                                                                            if($red_cnt >0){
                                                                $data_str .=                       '('.($red_cnt).') ';    
                                                                                            }
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <span style="color:#065A95;font-family: Arial,Helvetica,sans-serif;" >
                                                                ...
                                                                if(Yii::app()->session['user_id']==$alltopic->user_id){
                                                                    $data_str .= '<span class="c-edit-post-button" data-id="'.$alltopic->id.'" style="float:right; cursor:pointer; color:#999999; margin-right:8px;">Edit</span>';
 //                                                                   $data_str .= '<span class="c-edit-post-button" data-id="'.$alltopic->id.'" style="float:right; cursor:pointer;  ; color:#999999; margin-right:8px;">Hide</span>';
//<style="cursor: pointer;;float:right;color: #999999;; ;" onclick="setFlagMessage('.$alltopic->id.',\'Red\');">Report ';
                                                                }
                                                            $data_str .= '</span>
                                                        </td>
                                                    </tr>
...
//echo 1;die;
    ...
    return $d;
		...
		print_r(json_encode(utf8ize($response_aray)));exit;
	}    
        ...
        $user_comment_model = new AllPosts(); 
	        if(isset($_POST['AllPosts'])){
	            ...
	            $user_comment_model->save(false);
                    ...
                    $commentId = $_POST['AllPosts']['comment_id'];
                    $userId = $this->data['user_id'];
                    $repPost = AllPosts::model()->findByPk($commentId);
                    $topicId = $user_comment_model->main_id;
                    if($repPost->user_id!=$userId)
                    {
                        if($repPost->user_id==1)
                        {
                            $user1 = Admin::model()->findByPk($repPost->user_id);
                            $username1 = $user1->login_username;
                        }
                        else
                        {
                            $user1 = Users::model()->findByPk($repPost->user_id);
                            $username1 = $user1->username;
                        }
                        
                        if($userId==1)
                        {
                            $user2 = Admin::model()->findByPk($userId);
                            $username2 = $user2->login_username;
                        }
                        else
                        {
                            $user2 = Users::model()->findByPk($userId);
                            $username2 = $user2->username;
                        }
                        
                        $topic = Topics::model()->findByPk($topicId);
                        $reply_email = EmailTemplates::model()->findByPk(4);
                        ...
                        $link = Yii::app()->createAbsoluteUrl('topics/Viewtopic',array('topic_id'=>$topicId));

                        $content = str_replace("#Username1#",$username1,$reply_email->description);
                        $content = str_replace("#Username2#",$username2,$content);
                        $content = str_replace("#topiclink#",$link,$content);
                        $content = str_replace("#Topicname#",$topic->topic_title,$content);
                        $this->data['content'] = $content;                            
                        
                        //$email_flag = mail($to,$subject,$content);
