...
class Users extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            ...
            'user_all_post_green_flag_relation'=>array(self::HAS_MANY,'AllPostsFlags','user_id','condition'=>'flag_type="Green"'),
            'user_all_post_red_flag_relation'=>array(self::HAS_MANY,'AllPostsFlags','user_id','condition'=>'flag_type="Red"'),
            'user_all_post_block_flag_relation'=>array(self::HAS_MANY,'AllPostsFlags','user_id','condition'=>'block_user=1'),
            'user_all_post_hide_post_flag_relation'=>array(self::HAS_MANY,'AllPostsFlags','user_id','condition'=>'hide_post=1'),
	...
	public function search()
	{
        ...
        $sort->attributes = array( 
			...
			'red_flag'=>array(
                'asc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Red" LIMIT 0,1) ASC',
                'desc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Red" LIMIT 0,1) DESC',
            ),

            'red_flag_avg'=>array(
                'asc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Red" LIMIT 0,1) * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) ASC',
                'desc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Red" LIMIT 0,1) * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) DESC',
            ),
                        
            'block_flag'=>array(
                'asc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.block_user=1 LIMIT 0,1) ASC',
                'desc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.block_user=1 LIMIT 0,1) DESC',
            ),
            
            'block_flag_avg'=>array(
                'asc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.block_user=1 LIMIT 0,1)  * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) ASC',
                'desc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.block_user=1 LIMIT 0,1)  * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) DESC',
            ),
            
            'green_flag'=>array(
                'asc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Green" LIMIT 0,1) ASC',
                'desc'=>'(SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Green" LIMIT 0,1) DESC',
            ),
            
            'green_flag_avg'=>array(
                'asc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Green" LIMIT 0,1) * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) ASC',
                'desc'=>'((SELECT COUNT(user_id) from all_posts_flags WHERE all_posts_flags.user_id = t.id AND all_posts_flags.flag_type="Green" LIMIT 0,1) * 100/(SELECT COUNT(user_id) from all_posts WHERE all_posts.user_id = t.id LIMIT 0,1)) DESC',
            ),
            'agree'=>array(
