<div class="main_mid1" id="toggle_column" style="width:100%;display: none;margin-left:0px;">
    ...
    <div class="topic1" >
        <div class="profile-out-div"> 
            ...
            ?>
            <? if($user_model->facebook_id != 0 &&  $user_model->facebook_id!=""){
                    if($user_model->profile_image==""){
                         $Src= 'https://graph.facebook.com/'.$user_model->facebook_id.'/picture?type=large' ;
                    }
            }?>
            <? if($user_model->twitter_id != 0 &&  $user_model->twitter_id!=""){
               ...
               <p style="padding-top: 20px;"> 
                   ...
                   <?php if($green_total_cooment > 0){?>
                            <a class="people_label_value_right" style="color: white;" href="<?php echo Yii::app()->createUrl('site/Viewpeople',array('people_id'=>$user_model->id,'type'=>'green'))?>">
                                <div style="background-color:#07D000; color:white; font-size:11px; width:20px;height:12px; float:left; text-align:center; margin-right:1%;margin-top:5px" title="<?php echo $green_total_cooment;?> Green Flags">
                                    <div style="margin-top:-3px; font-size:11px;"><?php echo $green_total_cooment;?></div>
                                </div>
                            </a>
                   <?php }else{ ?>
                                <div class="people_label_value_right" style="background-color:#07D000; color:white; font-size:11px; width:20px;height:12px; float:left; text-align:center; margin-right:1%;margin-top:5px" title="<?php echo $green_total_cooment;?> Green Flags">
                   ...
                   <?php }
                        if($red_total_cooment > 0){ ?>
                            <a class="people_label_value_right" href="<?php echo Yii::app()->createUrl('site/Viewpeople',array('people_id'=>$user_model->id,'type'=>'red'))?>">
                                <div style="background-color:#FA3002; color:white; font-size:11px; width:20px;height:12px; float:left; text-align:center;margin-top:5px" title="<?php echo $red_total_cooment;?> Red  Flags">
                                    <div style="margin-top:-3px; font-size:11px;"><?php echo $red_total_cooment;?></div>
                                </div>
                            </a>
                   <?php }else{ ?>
                                <div class="people_label_value_right" style="background-color:#FA3002; color:white; font-size:11px; width:20px;height:12px; float:left; text-align:center;margin-top:5px" title="<?php echo $red_total_cooment;?> Red  Flags">
                                    <div style="margin-top:-3px; font-size:11px;">0</div>
                                </div>
                   <?php } ?>
                </p>
