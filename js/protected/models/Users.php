...
class Users extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('email','email'),
			array('group_id,facebook_id, twitter_id, username,profile_image, password, email, facebook_link, twitter_link, website_link', 'length', 'max'=>255),
			array('status', 'length', 'max'=>8),
			array('ip_address','safe'),
			array('created_date','default','value'=>new CDbExpression('NOW()'),'setOnEmpty'=>false),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('disagree,agree,green_flag_avg,green_flag,block_flag_avg,block_flag,red_flag_avg,red_flag,posts,id,group_id, facebook_id, twitter_id, username,profile_image, password, email, user_description, facebook_link, twitter_link, website_link, created_date, status,token,aiia_discriptor,favorite_rule,category_groups_id,category_groups_id_hide', 'safe', 'on'=>'search'),
		);
	}
