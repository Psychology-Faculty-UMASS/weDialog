...
class EmailTemplates extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name,subject,description', 'required'),
			array('name', 'length', 'max'=>128),
			array('subject', 'length', 'max'=>256),
			array('notification_flag','safe'),
			array('name,description,Subject', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, subject, description', 'safe', 'on'=>'search'),
		);
	}
